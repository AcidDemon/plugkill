...
pub struct ThunderboltDeviceInfo {
    ...
    /// The sysfs name under `/sys/bus/thunderbolt/devices`: `<domain>-<route>`,
    /// such as `0-1` for a device on the host router and `0-301` for one
    /// behind it. The route is packed into the digits, with no separator, so
    /// the name does not spell the chain out. Topology only, never identity.
    /// None where there is no sysfs to read it from.
...
#[cfg(test)]
impl Default for ThunderboltSnapshot {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ThunderboltSnapshot {
    ...
    ///
    /// Presence/absence by unique_id; no count logic.
...
/// Enumerate all currently connected Thunderbolt devices.
...
pub(crate) fn enumerate_thunderbolt_devices_from(
    sysfs_root: &Path,
) -> Result<ThunderboltSnapshot, Error> {
...
// FreeBSD exposes no userspace equivalent of Linux's per-device unique_id, so
// there is nothing to enumerate. The bus reports empty and warns once; config
// validation also flags watch_thunderbolt on FreeBSD.
#[cfg(target_os = "freebsd")]
mod freebsd {
    ...
    fn warn_once() {
        ...
        ONCE.call_once(|| {
            warn!("thunderbolt monitoring is unavailable on FreeBSD (no per-device unique_id)");
        });
    }
...
#[cfg(any(target_os = "linux", test))]
pub(crate) fn enumerate_thunderbolt_devices_detailed_from(
    sysfs_root: &Path,
...
mod tests {
    ...
    fn test_enumerate_detailed_from_mock_sysfs() {
        ...
        // The sysfs name is the topology: domain 0, route 1.
        assert_eq!(d.port.as_deref(), Some("0-1"));
    }
