...
#[cfg(test)]
impl Default for SdCardSnapshot {
    fn default() -> Self {
        Self::new()
    }
}
...
/// Enumerate all currently connected SD/MMC cards.
...
/// Pull a card serial from an mmcsd newbus pnpinfo string (space-separated
/// `key=value` tokens). Returns None when no serial field is present.
#[cfg(any(target_os = "freebsd", test))]
fn parse_mmcsd_pnpinfo(pnpinfo: &str) -> Option<String> {
    pnpinfo
        .split_whitespace()
        .find_map(|tok| {
            let (k, v) = tok.split_once('=')?;
            matches!(k, "sn" | "serial" | "SN").then(|| v.trim_matches('"').to_string())
        })
        .filter(|s| !s.is_empty())
}
...
pub(crate) fn enumerate_sdcard_devices_from(sysfs_root: &Path) -> Result<SdCardSnapshot, Error> {
...
// FreeBSD has no /sys/bus/mmc. Cards are enumerated from /dev/mmcsdN nodes and
// keyed by the CID serial number pulled from dev.mmcsd.N.%pnpinfo.
//
// ponytail: the exact pnpinfo field name is unverified without an SD card on a
// FreeBSD box. If parse_mmcsd_pnpinfo returns None the card falls back to a
// device-node key and warns once, so whitelist-by-serial stays stable only once
// the real field is confirmed. Fix the token match in parse_mmcsd_pnpinfo then.
#[cfg(target_os = "freebsd")]
mod freebsd {
    use super::{SdCardDeviceId, SdCardDeviceInfo, SdCardSnapshot, parse_mmcsd_pnpinfo};
    use crate::error::Error;
    ...
    fn mmcsd_units() -> Vec<u32> {
        let mut units = Vec::new();
        if let Ok(rd) = std::fs::read_dir("/dev") {
            for entry in rd.flatten() {
                let name = entry.file_name();
                let name = name.to_string_lossy();
                if let Some(rest) = name.strip_prefix("mmcsd")
                    && !rest.is_empty()
    ...
    fn card_serial(n: u32) -> Option<String> {
        let pnp = sysctl_string(&format!("dev.mmcsd.{n}.%pnpinfo"))?;
        parse_mmcsd_pnpinfo(&pnp)
    }
    ...
    fn warn_fallback() {
        ...
        ONCE.call_once(|| {
            warn!(
                "could not read an SD card serial from dev.mmcsd.*.%pnpinfo; \
                 falling back to device-node identity (whitelist-by-serial unstable)"
            );
        });
    }

    fn serial_for(n: u32) -> String {
        card_serial(n).unwrap_or_else(|| {
            warn_fallback();
            format!("mmcsd{n}")
        })
    }
    ...
    pub fn enumerate_detailed() -> Result<Vec<SdCardDeviceInfo>, Error> {
        let mut out = Vec::new();
        for n in mmcsd_units() {
            out.push(SdCardDeviceInfo {
                serial: serial_for(n),
                name: Some(format!("mmcsd{n}")),
                card_type: sysctl_string(&format!("dev.mmcsd.{n}.%desc")),
                cid: None,
                manfid: None,
                oemid: None,
                date: None,
...
#[cfg(any(target_os = "linux", test))]
pub(crate) fn enumerate_sdcard_devices_detailed_from(
    sysfs_root: &Path,
...
mod tests {
    ...
    #[test]
    fn test_parse_mmcsd_pnpinfo() {
        assert_eq!(
            parse_mmcsd_pnpinfo("vendor=0x0000 sn=0x12345678 hwrev=0x8"),
            Some("0x12345678".to_string())
        );
        assert_eq!(
            parse_mmcsd_pnpinfo("serial=\"abc123\" oem=0x5344"),
            Some("abc123".to_string())
        );
        assert_eq!(parse_mmcsd_pnpinfo("vendor=0x0000 hwrev=0x8"), None);
        assert_eq!(parse_mmcsd_pnpinfo(""), None);
    }
}
