...
impl PciSnapshot {
    ...
    /// The selectors in this snapshot, for a client that wants to list what the
    /// bus currently holds rather than only count it.
...
fn parse_pciconf(output: &str, ignore: &[String]) -> HashSet<String> {
    output
        .lines()
