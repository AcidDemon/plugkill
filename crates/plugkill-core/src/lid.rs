...
/// The lid state a procfs file reports. Split out so a test can point it
/// at a file it wrote, rather than at the one ACPI owns.
#[cfg(any(target_os = "linux", test))]
pub(crate) fn read_lid_state_from_proc(path: &Path) -> LidState {
...
fn parse_lid_devd_line(line: &str) -> Option<LidState> {
    ...
    fn listen_loop() {
        ...
        loop {
            match UnixStream::connect(DEVD_PIPE) {
                ...
                Err(e) => {
                    WARNED.call_once(|| {
                        warn!("lid monitoring: cannot connect to devd at {DEVD_PIPE}: {e}")
                    });
                }
            }
