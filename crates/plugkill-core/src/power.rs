...
fn parse_acline(acline: Option<i64>) -> PowerState {
    match acline {
...
/// The power state under a given sysfs root. Split out so a test can point
/// it at a tree it wrote.
#[cfg(any(target_os = "linux", test))]
pub(crate) fn read_power_state_from(sysfs_root: &Path) -> PowerState {
