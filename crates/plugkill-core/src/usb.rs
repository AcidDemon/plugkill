...
pub struct UsbDeviceInfo {
    ...
    /// The sysfs port path: the directory name under `/sys/bus/usb/devices`,
    /// such as `usb2` for a root hub, `2-3` for a device on it and `2-3.4` for
    /// one behind that hub. Topology only, never identity. None where there is
    /// no sysfs to read it from.
...
pub fn enumerate_devices() -> Result<DeviceSnapshot, Error> {
    ...
    {
        Ok(DeviceSnapshot::from_map(HashMap::new()))
    }
}

/// Enumerate USB devices from a custom sysfs root (for testing).
#[cfg(any(target_os = "linux", test))]
pub(crate) fn enumerate_devices_from(sysfs_root: &Path) -> Result<DeviceSnapshot, Error> {
    ...
    for entry in entries {
        ...
        // Skip entries missing idVendor or idProduct.
...
/// Enumerate all connected USB devices with extended info for display.
...
pub fn enumerate_devices_detailed_from(sysfs_root: &Path) -> Result<Vec<UsbDeviceInfo>, Error> {
    ...
    for entry in entries {
        ...
        // An interface directory (`2-3:1.0`) is not a device. It carries no
        // idVendor either, but skipping it by name keeps it out of the
        // topology for certain.
        if port.contains(':') {
            continue;
        }
...
#[cfg(target_os = "freebsd")]
mod freebsd {
    use super::{DeviceSnapshot, UsbDeviceId, UsbDeviceInfo};
    use crate::error::Error;
    use log::warn;
    use std::collections::HashMap;

    /// libusb reports vendor/product as u16; format as 4-char lowercase hex to
    /// match the Linux sysfs `idVendor`/`idProduct` values, so whitelists stay
    /// portable across both platforms.
    pub fn enumerate() -> Result<DeviceSnapshot, Error> {
        let list = rusb::devices().map_err(|e| Error::Usb(format!("libusb: {e}")))?;
        let mut devices: HashMap<UsbDeviceId, u32> = HashMap::new();
        for dev in list.iter() {
            let desc = match dev.device_descriptor() {
                Ok(d) => d,
                Err(e) => {
                    warn!("cannot read USB device descriptor: {e}");
                    continue;
                }
            };
            let id = UsbDeviceId {
        ...
        Ok(DeviceSnapshot::from_map(devices))
    }

    pub fn enumerate_detailed() -> Result<Vec<UsbDeviceInfo>, Error> {
        let list = rusb::devices().map_err(|e| Error::Usb(format!("libusb: {e}")))?;
        let mut out = Vec::new();
        for dev in list.iter() {
            let desc = match dev.device_descriptor() {
            ...
            // String descriptors need an open handle. Without device permissions
            // open() fails and those fields stay None, the same degradation as a
            // missing sysfs attribute on Linux.
            let handle = dev.open().ok();
            let (manufacturer, product, serial) = match &handle {
                Some(h) => (
                    h.read_manufacturer_string_ascii(&desc)
                        .ok()
                        .filter(|s| !s.is_empty()),
            ...
            // Emit the same Mbps strings sysfs uses so format_speed() renders
            // identically on both platforms.
            let speed = match dev.speed() {
                rusb::Speed::Low => Some("1.5".to_string()),
                rusb::Speed::Full => Some("12".to_string()),
                rusb::Speed::High => Some("480".to_string()),
                rusb::Speed::Super => Some("5000".to_string()),
                rusb::Speed::SuperPlus => Some("10000".to_string()),
                _ => None,
            };
            ...
            out.push(UsbDeviceInfo {
                ...
                serial,
                speed,
                busnum: Some(dev.bus_number().to_string()),
                devnum: Some(dev.address().to_string()),
                // FreeBSD has no sysfs port path.
                port: None,
            });
        }
        ...
        out.sort_by(|a, b| {
            ...
            (parse(&a.busnum), parse(&a.devnum)).cmp(&(parse(&b.busnum), parse(&b.devnum)))
        });
        Ok(out)
    }
}
...
mod tests {
    ...
    fn test_enumerate_detailed_full_attributes() {
        ...
        // The sysfs directory name, which is what encodes the tree.
        assert_eq!(d.port.as_deref(), Some("1-1"));
    }
