...
fn default_plugkill_socket() -> PathBuf {
    PathBuf::from(plugkill_core::ipc::DEFAULT_SOCKET_PATH)
}
...
# Path to plugkill's Unix socket. Omit to use the OS default:
# /run/plugkill on Linux, /var/run/plugkill on FreeBSD.
